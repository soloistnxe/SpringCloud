...
public class AprioriMyself {
    ...
    public  int times = 0;//迭代次数
    private  double MIN_SUPPROT = 0.5;//最小支持度百分比
    private  double MIN_CONFIDENCE = 0.75;//最小置信度
    private  boolean endTag = false;//循环状态，迭代标识
     List<List<String>> record = new ArrayList<List<String>>();//数据集
     List<List<String>> frequentItemset = new ArrayList<>();//存储所有的频繁项集
     List<Mymap> map = new ArrayList();//存放频繁项集和对应的支持度技术
   ...
   /* public static void main(String args[]) {
        ...
        *//*************读取数据集**************//*
        List<List<String>> record = aprioriMyself.getRecord("D:/top1000data");
        //控制台输出记录
        System.out.println("读取数据集record成功===================================");
        Map<String, Double> recommend = aprioriMyself.getRecommend(record);
        Set<String> strings = recommend.keySet();
        for (String string : strings) {
            System.out.println(string + ":" + recommend.get(string));
        }
    }*/
    ...
    public  int getCount(List<String> in)//根据频繁项集得到 其支持度计数
    {
        int rt = 0;
     ...
     * @return
     */
    private  List<List<String>> getSupprotedItemset(List<List<String>> CandidateItemset) { //对所有的商品进行支持度计数
    ...
    public List<Itemcount> convertCount(List<Mymap> map){
        List<Itemcount> itemcounts = new ArrayList<>();
        for (Mymap mymap : map) {
            Itemcount itemcount = new Itemcount();
            List<String> li = mymap.getLi();
            String name = "";
            for (int i = 0; i < li.size()-1; i++) {
                name+=li.get(i)+"，";
            }
            name+=li.get(li.size()-1);
            itemcount.setName(name);
            itemcount.setValue(mymap.getcount());
